        ...
        for row in range(self.dim_size):
            for col in range(self.dim_size):
                if (row,col) in self.dug:
                    visible_board[row][col] = str(self.board[row][col]) #user see the number that we dug
                else:
                    visible_board[row][col] = ' ' # if its not dug  user see nothing
...
def play(dim_size=10, num_bombs=10):
    ...
    while len(board.dug) < board.dim_size ** 2 - num_bombs:
        ...
        #checking if the user put something bigger than dim_siize or smaller than zero 
    ...
    if safe:
        ...
        board.dug = [(r,c) for r in range(board.dim_size) for c in range(board.dim_size)] #digging everything on a board , after  the game is over 
        print(board)
